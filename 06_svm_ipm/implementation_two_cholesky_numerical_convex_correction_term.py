...
def primal_dual_path(X_train, y_train):
    ...
    #CHOLESKY ADDITION TERM, to guarantee this Matrix is Positive Semi Defined, So that none of the eigen values 
    # will be negative (none of the linear transformations caused by any of the columns would result in changing its orientation in
    # 180 degrees ?
    CHOLESKY_ADDITION_TERM = np.identity(npoints) + 1E-08
    Q = ( y_train @ y_train.T ) * (X_train @ X_train.T) + CHOLESKY_ADDITION_TERM

    # check eigenvalues
    eigen_values = np.linalg.eigvals(Q)
    boolean_mask_negative_eigen_vals = (eigen_values<0)
    if sum(boolean_mask_negative_eigen_vals) > 0:
        print(f'Negative eigen values found. So interpreter will exit')
        print(eigen_values)
        exit()
    else:
        print(f'No negative eigenvalues found') 
    # print(Q)
    # print(Q.shape)
    ...
    df_plot = pd.DataFrame({'x1':X_train[:,0],'x2':X_train[:,1],'label':colors})
    p = (
